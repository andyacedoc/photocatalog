...
class Catalog {
    ...
    private function editForm() //первоначальный вывод формы редактирования данных фотографии на страницу
	{
			...
			$htmlEditForm = '<div class="col-12">Введите идентификатор фотографии 
							...
							<span style="float:right;">Добавить фотографию 
							<input type="button" name="button" value="Добавить" onclick="sendRequestPhoto(\'' . $urlCreate . '\', \'emptypost\', \'' . $formssite . '\');"></span></div>';	
	return $htmlEditForm;
	}
	...
	public function read()
    {
		...
		$htmlAdminMenu = ''; //Проверка открытой сессии и вывод меню редактирования для админа на странице menubar.php
		...
		if ($Idcatalog !== 0) {
			$htmlParentWay = '&nbsp; &nbsp;';
			if (!empty($this->model->getData('dataParentCatalog'))) { //формирование пути дерева каталога
					...
					foreach ($this->model->getData('dataPhoto') as $catalog) {
						$htmlRender .= '<div class="col">
										...
										<div class="col">'
											...
											. '<br><br><br><input type="' . $button . '" name="button" value="Редактировать" onclick="showtEditPhotoForm(' . $catalog['idphoto'] . ', \'' . $urlRead . '\');">' .
										'</div>';
					}									
				...
				if (!empty($this->model->getData('dataSubCatalog'))){
					...
					if (!empty($this->model->getData('dataPhoto'))) {
						...
						if (!empty($this->model->getError())) {
							echo '<h5>&nbsp; &nbsp;' . $this->model->getError() . '</h5>';
						}					
						foreach ($this->model->getData('dataPhoto') as $catalog) {
							$htmlRender .= '<div class="col"> <figure class="figure">
											<a href="' . BASEURL . 'catalog/read/'
